...
class PipelineRunner:
    ...
    def run_vina(self) -> tuple[int, int]:
        ...
        self._emit("stage_started", "vina", total=len(states))
        for index, state in enumerate(states, 1):
            self._emit("item_started", "vina", state["state_id"], index, len(states))
            ...
            settings_fp = fingerprint(settings=run_settings, inputs={"receptor": receptor_hash}, tool_version=str(executable))
            with self.repository.connection() as conn:
                reusable = conn.execute("""SELECT d.run_id, raw.relative_path AS raw_path, log.relative_path AS log_path
                    ...
                    WHERE d.state_id=? AND d.settings_fingerprint=? AND d.status=? AND d.is_current=1""", (state["state_id"], settings_fp, StageStatus.COMPLETED)).fetchone()
            if reusable and reusable["raw_path"] and reusable["log_path"] and (self.repository.root / reusable["raw_path"]).is_file() and (self.repository.root / reusable["log_path"]).is_file():
                succeeded += 1
                self._emit("item_skipped", "vina", state["state_id"], index, len(states), skipped=1, message="Matching docking run reused")
                continue
            try:
                with self.repository.connection() as conn:
                    conn.execute("INSERT INTO docking_runs(run_id,state_id,receptor_hash,settings_fingerprint,status,command_json,started_at) VALUES (?, ?, ?, ?, ?, ?, ?)", (run_id, state["state_id"], receptor_hash, settings_fp, StageStatus.RUNNING, json.dumps(run_settings), utc_now()))
                ...
                self._emit("item_succeeded" if result.poses else "item_failed", "vina", state["state_id"], index, len(states), succeeded=succeeded, failed=failed, message="" if result.poses else "No valid Vina poses")
            except Exception as exc:
                failed += 1
                with self.repository.connection() as conn:
                    conn.execute("UPDATE docking_runs SET status=?, ended_at=? WHERE run_id=?", (StageStatus.FAILED, utc_now(), run_id))
                self._emit("item_failed", "vina", state["state_id"], index, len(states), failed=failed, message=str(exc))
        self._emit("stage_completed", "vina", total=len(states), succeeded=succeeded, failed=failed)
    ...
    def run_postdock(self) -> tuple[int, int]:
        ...
        success = failed = 0
        for run in runs:
            already_exported = any((self.repository.root / folder / run["parent_id"]).exists() for folder in ("For_PostDocking/SDF", "For_PostDocking/PDBQTs"))
            if already_exported:
            ...
            base_dir = self.repository.root / "For_PostDocking"
